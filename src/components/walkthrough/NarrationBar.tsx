...
export default function NarrationBar({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-[80px] z-[9500] bg-surface border-t border-border animate-slide-up-bar">
      <div className="h-full max-w-[1200px] mx-auto px-6 flex items-center gap-6">
        {/* Back */}
        <button
          onClick={onPrev}
          disabled={isFirst}
          className="flex items-center gap-1 text-xs text-secondary hover:text-primary disabled:text-tertiary disabled:cursor-not-allowed transition-colors flex-shrink-0"
        >
          <ChevronLeft size={14} />
          Back
        </button>
        ...
        <button
          onClick={onNext}
          className={`flex items-center gap-1 text-xs font-medium px-4 py-1.5 rounded transition-colors flex-shrink-0 ${
            isWaiting
        ...
        <button
          onClick={onClose}
          className="w-6 h-6 flex items-center justify-center rounded text-tertiary hover:text-secondary hover:bg-hover transition-colors flex-shrink-0"
        >
          <X size={14} />
        </button>
      </div>
